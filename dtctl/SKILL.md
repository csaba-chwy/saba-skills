...
4. Use at most one additional simple metric query when a scalar total or regional comparison materially improves the summary. When direct Dynatrace proof is useful, make that second query a scalar `summarize` and link it as a readable bar chart; never substitute a native-array table for the inline trend visualization.
...
For a quick rundown, treat the inline three-panel timeline as the primary visualization and a summarized Dynatrace bar-chart link as optional proof. Do not link the native arrays as a table. Read [references/query-strategy.md](references/query-strategy.md) for the reusable builder command and adaptation examples.
