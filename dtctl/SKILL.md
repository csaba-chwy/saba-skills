...
### Generate evidence links without a browser

Prefer Dynatrace intent URLs generated by `dtctl`; do not hand-construct application routes or use Chrome merely to create or validate evidence links. `dtctl open intent` prints the URL unless `--browser` is explicitly supplied.

Use the most specific supported intent for the evidence:

```bash
# Exact trace. The trace ID comes from the observed query result.
dtctl --context "$DT_CONTEXT" open intent \
  dynatrace.distributedtracing/view-trace \
  --data trace_id="TRACE-ID" \
  --plain

# Exact DQL evidence for logs, metrics, or a selective trace query.
# Write the payload to a private temporary file and delete it after generating the URL.
dtctl --context "$DT_CONTEXT" open intent \
...
Use this payload shape for query evidence:
...
- Metrics: link the exact metric DQL, dimensions, and absolute timeframe through `dynatrace.notebooks/view-query`. Use a Data Explorer share link only when the user specifically needs that visualization and supplies or requests browser interaction.
- Traces: prefer `dynatrace.distributedtracing/view-trace` for a known `trace.id`. Use `dynatrace.notebooks/view-query` for a selective multi-trace query or when the trace intent cannot express the required context.
- Logs: use `dynatrace.notebooks/view-query` for a filtered log set. For one exact record, use `dynatrace.logs/view-log-entry` only after `dtctl describe intent dynatrace.logs/view-log-entry` confirms the required payload fields in the selected context.

Where permissions allow, use `dtctl describe intent APP-ID/INTENT-ID` to confirm the target intent and its payload schema. Treat `403 Forbidden` as an intent-discovery permission limitation, not proof that the intent or target app is absent. Known intent names documented by the installed `dtctl` help may still be used, but label the URL as generated and not UI-opened when discovery could not confirm it.

Validate generated links without opening them:

- Confirm the URL hostname matches the environment URL returned for `DT_CONTEXT`.
- Confirm the path starts with the Dynatrace AppShell intent route and names the intended app and intent.
- Decode the fragment locally and compare it with the exact trace ID or DQL and absolute timeframe used for the evidence.
- Keep the observed `dtctl query` values as the proof. A generated link makes that proof reproducible; the link alone does not prove that the query returned data.

Do not invoke a browser tool on the normal path. Open a link only when the user explicitly asks for browser validation or reports that it does not resolve. If a recipient app is unavailable, fall back to the generic `dynatrace.notebooks/view-query` intent when it can reproduce the claim; otherwise label the finding as **unlinked interim evidence**.
