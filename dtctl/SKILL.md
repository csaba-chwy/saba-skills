---
name: dtctl
description: Investigate Dynatrace services with dtctl using the correct nonprod or prod context, a concise metric-link fast path for quick service rundowns, metric-first failure discovery, safe bounded log and trace queries, early trace-query links, and parallel evidence collection. Use for quick health summaries, error analysis, trace-to-log correlation, deployment symptoms, Kubernetes workload logs, service latency, and service-specific observability investigations over short or long time ranges.
---
...
- Do not invoke client-side visualization, image-generation, browser, or computer-use capabilities to present or validate telemetry. Do not create HTML, SVG, canvas, or raster chart assets; render or open images; inspect screenshots; or manually redraw metric arrays. Dynatrace evidence links are the only visualization surface for this skill.
...
2. Summarize the returned arrays directly in prose. Report the target, absolute window, interval, units, headline totals or ranges, and the timing of material peaks or dips without creating a local or inline chart.
...
4. Make Dynatrace graphs the primary evidence. Generate separate request-count, error-rate, and p95-latency links with `scripts/src/build_logs_events_graph_link.py`, retaining the exact selector, absolute window, interval, grouping dimensions, and native array for each measure. Each graph must contain compatible units and preserve time on the x-axis; never combine measures whose scales make the result hard to read. Derive these graph queries from the successful aligned query without rerunning telemetry solely for link creation.
...
6. Return the timeframe, two or three headline observations, and the three descriptively named Dynatrace graph links beside the claims they support. Do not add a proof table unless it is necessary to prevent ambiguity.
