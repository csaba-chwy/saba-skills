...
1. Resolve and authenticate the target safely, then run no more than two simple metric queries. Start with one `dt.service.request.count` query grouped only by a small useful dimension such as regional `service.name` or `failed`, with an explicit interval appropriate to the requested window. Preserve the native `timeframe`, `interval`, and numeric arrays so the linked graph uses time on the x-axis. Optionally use the second query for a basic latency or error comparison when it materially improves the summary.
2. Prefer a readable time-series bar-chart link over exhaustive endpoint rankings, catalog discovery, custom service metrics, or multiple companion queries. Compute simple headline totals or comparisons from the returned arrays for prose, but keep the linked DQL time-bucketed; do not chase bucket-edge precision for a quick summary.
