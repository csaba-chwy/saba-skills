...
- Select all regional lifecycle series through the shared logical metric selector. Group by `service.name` only when a regional cross-check is needed; omit it to aggregate regions in one query.
