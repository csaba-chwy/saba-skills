...
- Use the exact tagged `cart-spa` name and workload when falling back from the shared selector. Exact-name discovery can return several entities; rank them by current request count and keep more than one only if the investigation covers distinct active traffic.
