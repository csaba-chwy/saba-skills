...
- `dt.service.request.count` exposes `primary_tags.version`; observed values such as `0.180.0` match the application's GitHub tag version. Filter by the exact value and group by regional `service.name` to locate first traffic for a deployment.
