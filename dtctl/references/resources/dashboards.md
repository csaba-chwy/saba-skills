...
dtctl query 'fetch logs, from:now()-15m | filter dt.entity.service == "SERVICE-xxx" | filter loglevel == "ERROR" | summarize cnt = count()' --default-scan-limit-gbytes 5 -o json --plain

# Verify time series output has the expected shape
dtctl query 'fetch logs, from:now()-15m | filter dt.entity.service == "SERVICE-xxx" | makeTimeseries cnt = count(), interval:2m, by:{k8s.container.name}' --default-scan-limit-gbytes 5 -o json --plain
```
...
fetch spans, from:now()-15m | filter dt.entity.service == "SERVICE-xxx" | summarize errors_5xx = countIf(http.response.status_code >= 500), total = count()
             | fieldsAdd error_rate = 100.0 * errors_5xx / total

-- For non-HTTP/gRPC services: span status may work
fetch spans, from:now()-15m | filter dt.entity.service == "SERVICE-xxx" | summarize errors = countIf(status == "ERROR")

-- Best coverage: combine both signals
fetch spans, from:now()-15m | filter dt.entity.service == "SERVICE-xxx" | summarize errors = countIf(status == "ERROR" or http.response.status_code >= 500), total = count()
