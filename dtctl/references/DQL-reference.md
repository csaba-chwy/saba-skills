...
For every `fetch`, specify a narrow time range, filter on a known entity or another selective attribute before sorting, select only required fields, and finish with a result `limit`. Start with a 15-minute window and `limit 20` for ad-hoc troubleshooting. A result limit does not limit Grail scan volume, so execute with `--default-scan-limit-gbytes 5` and narrow the query—not the guardrail—when it reaches the cap.
...
```dql
-- Source forms only. Do not execute without a target filter, selected fields, and result limit.
fetch logs, from:now()-15m          -- Log records
fetch events, from:now()-15m        -- System events
fetch bizevents, from:now()-15m     -- Business events
fetch spans, from:now()-15m         -- Trace spans
fetch security.events, from:now()-15m -- Security/vulnerability events
...
timeseries avg(dt.host.cpu.usage), from:now()-2h, interval:5m -- Metrics (NOT fetch metrics)
```
...
```dql
timeseries avg(dt.host.cpu.usage), by:{dt.entity.host}, from:now()-2h, interval:5m
```
